...
public class MainActivity extends AppCompatActivity {
    ...
    public void clearClickHandler(Button btnClear) {
        ...
        //Clears amount spent
        tvSpent.setText(R.string.zero_spent);
        spent = 0;
    }
    ...
    private void setupRecyclerView() {
        ...
        //set up Spent if items are saved
        setUpSpent(shoppingItemList);
    }
    ...
    /*
    Set spent amount from saved items
     */
    public void setUpSpent(List<ShoppingItem> shoppingItemList) {
        for (ShoppingItem item : shoppingItemList) {
            if (item.isBought()) {
                int cost = Integer.parseInt(item.getShoppingItemCost());
                updateSpent(cost);
            }
        }
    }
